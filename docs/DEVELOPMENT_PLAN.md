...
3. AI 在 `strategy-space-v1` 中输出 `OPEN_LONG` 或 `NO_TRADE` 等合法 Strategy Intent。
...
flowchart TD
    ...
    P304["P3-04 DeepSeek Strategy Intent"]
...
- **任务**：纯领域类型、稳定 ID、合法组合、状态迁移和属性测试。
- **依赖**：`P0-03`,`P1-01`。
- **验收**：金额无 `f64`；Spot 不可 OpenShort；未知枚举拒绝；Strategy Intent 不能携带自由绝对价格、数量、杠杆或执行权限；非法状态迁移 fail closed。
...
- **任务**：replay clock、market manifest/hash、固定随机性和报告。
- **依赖**：`P2-03`,`P1-04`。
- **验收**：相同 manifest 两次结果 hash 一致；无 future candle；产物不包含 PnL 结论；manifest 不含新闻依赖。
...
- **目标**：建立不可绕过的风险裁决。
- **依赖**：`P1-02`,`P3-01`。
- **验收**：未获批动作不能构造 Execution 输入；只能批准、向下调整、拒绝或降权；不能替换 AI 策略。
...
- **依赖**：`P4-03`。
- **验收**：Prototype、Long-running Paper、Historical Strategy Evidence 和 Testnet Gate 全部独立通过；无未解释关键差异。
...
flowchart TD
    ...
    A["DeepSeek Strategy Intent v2"]
    V{"Schema / Semantic Validation"}
    M{"Deterministic Materialization"}
    R{"Risk Engine"}
